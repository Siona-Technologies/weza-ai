...
### The owner commands: `fix`, `review`, `summary`, `undo` and `receipt`
...
**`receipt`** (also `photo`, `image`, `picha`) sends the stored photo back into
the chat — the newest entry that has one, or, mid-review, the entry on screen.
That second case is where it earns its place: the owner has been asked to confirm
an amount and wants to see what it was read off. Looking at a receipt changes
nothing, so the walk stays exactly where it was, waiting for their answer.

The photo is sent as an attachment, not as a link. Twilio fetches the signed URL
server-side, so the image arrives in the conversation and stays there — owned by
WhatsApp rather than by us — and the Cloudinary link never reaches the owner's
phone.

It looks for the newest entry that actually *has* an image rather than simply the
newest entry, since most captures are text or voice with no picture at all.
Entries recorded before image storage existed say so plainly instead of failing.
...
the stored URL can't be fetched by anyone who has it; reading one needs a signed
link from `signedUrlFor()`. Cloudinary's default is public delivery, which would
put customer receipts — vendor, amounts, sometimes a KRA PIN — on a guessable
public address.

**That signed link does not expire.** The obvious `expires_at` option is silently
ignored by `cloudinary.url()` for signed delivery URLs — the output is identical
with an expiry, without one, and with a far-future one, which was checked rather
than assumed. Real expiry needs Cloudinary's token-based authentication, an
account-level feature we don't have. What protects the image instead is that the
URL can't be constructed without the API secret, and that the link never leaves
our infrastructure: `receipt` sends the photo, so Twilio fetches the URL
server-side and re-hosts it. The owner's phone never sees the Cloudinary link.
